...
class country extends CI_controller {
	...
	{
		parent::__construct();
		$this->load->model('admin/mdl_country','con');
		$this->con->cTableName = $this->cTable;
		$this->con->cAutoId = $this->cAutoId;
		$this->is_ajax = $this->input->is_ajax_request();
		
		if($this->input->get('item_id') != '' || $this->input->post('item_id') != '')
			$this->cPrimaryId  = $this->con->cPrimaryId = _de($this->security->xss_clean($_REQUEST['item_id']));
		$this->chk_permission();	
...
*/	
	...
	function countryName($str)
	{
		...
		$c = $this->db->where('country_name',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('countryName', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Country', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
			setFlashMessage('error',getErrorMessageFromCode('01010'));
			showPermissionDenied();
		}
		$num = $this->con->getData();
...
*/
	function countryForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('country_name','Country Name','trim|required|callback_countryName');
		...
		else
		{
			...
			{
				$this->con->cPrimaryId = $this->cPrimaryId; // setting variable to model
				$this->con->saveData();
