...
class customer_group extends CI_Controller {
	...
	var $is_ajax = false;
	var $cAutoId = 'customer_group_id';
	var $cPrimaryId = '';
	var $cTable = 'customer_group';
	var $controller = 'customer_group';
	var $per_add = 1;
	var $per_edit = 1;
	var $per_delete = 1;
	var $per_view = 1;
	...
	{
		parent::__construct();
		$this->load->model('admin/mdl_customer_group','cug');
		$this->cug->cTableName = $this->cTable;
		$this->cug->cAutoId = $this->cAutoId;
		$this->is_ajax = $this->input->is_ajax_request();
		
		if($this->input->get('item_id') != '' || $this->input->post('item_id') != '')
			$this->cPrimaryId  = $this->cug->cPrimaryId = _de($this->security->xss_clean($_REQUEST['item_id']));
		$this->chk_permission();		
...
*/	
	...
	function checkCustomerGroupKey($str)
	{
		if($this->cPrimaryId)
			$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
				
		$c = $this->db->where('customer_group_type',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('checkCustomerGroupKey', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
			return true;
	}	
	function customerGroupName($str)
	{
		if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('customer_group_name',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('customerGroupName', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Customer Group', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
			setFlashMessage('error',getErrorMessageFromCode('01010'));
			showPermissionDenied();
		}
		$num = $this->cug->getData();
...
*/
	function customerGroupForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('customer_group_name','Customer Group Name','trim|required|callback_customerGroupName');
		if(!$this->cPrimaryId)
			$this->form_validation->set_rules('customer_group_type',' Group Type','trim|required|callback_checkCustomerGroupKey');
		...
		else
		{
			...
			{
				$this->cug->cPrimaryId = $this->cPrimaryId; // setting variable to model
				$this->cug->saveData();
