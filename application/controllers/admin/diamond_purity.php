...
class diamond_purity extends CI_Controller {
	
	var $is_ajax = false;
	var $cAutoId = 'diamond_purity_id';
	var $cPrimaryId = '';
	var $cTable = 'diamond_purity';
	var $controller = 'diamond_purity';
	var $per_add = 1;
	var $per_edit = 1;
	var $per_delete = 1;
	var $per_view = 1;
	...
	function diamond_purity()
	{
		parent::__construct();
		$this->load->model('admin/mdl_diamond_purity','mc');
...
*/	
	...
	function diamondPurityName($str)
	{
		
		if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('diamond_purity_name',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('diamondPurityName', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
	}
	function diamondPurityKey($str)
	{
		if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('diamond_purity_key',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('diamondPurityKey', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Diamond Purity', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
...
*/
	function diamondPurityForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('diamond_purity_name','Diamond Purity name','trim|required|callback_diamondPurityName');
		if(!$this->cPrimaryId)
		{
			$this->form_validation->set_rules('diamond_purity_key','Diamond Purity Key','trim|required|callback_diamondPurityKey');
		}
 ...
 */	
	function updateSortOrder()
	{
		...
		if($id!='' && $sort_order!='')
		{
			$this->db->where($this->cAutoId,$id)->update($this->cTable,array('diamond_purity_sort_order'=>$sort_order));
