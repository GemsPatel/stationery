...
class diamond_type extends CI_Controller {
	
	var $is_ajax = false;
	var $cAutoId = 'diamond_type_id';
	var $cPrimaryId = '';
	var $cTable = 'diamond_type';
	var $controller = 'diamond_type';
	var $per_add = 1;
	var $per_edit = 1;
	var $per_delete = 1;
	var $per_view = 1;
	...
	function diamond_type()
	{
		parent::__construct();
		$this->load->model('admin/mdl_diamond_type','dt');
		$this->dt->cTableName = $this->cTable;
		$this->dt->cAutoId = $this->cAutoId;
		$this->is_ajax = $this->input->is_ajax_request();
		
		if($this->input->get('item_id') != '' || $this->input->post('item_id') != '')
			$this->cPrimaryId  = $this->dt->cPrimaryId = _de($this->security->xss_clean($_REQUEST['item_id']));
...
*/	
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Diamond Type', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
...
+-----------------------------------------+
*/
function checkDiamondType($str)
{
	if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
	$c = $this->db->where('diamond_type_name',$str)->get($this->cTable)->num_rows();
	if($c > 0)
	{
		$this->form_validation->set_message('checkDiamondType', 'This '.$str.' already exist in database, please try different.');
		return false;
	}
	else
		return true;
...
function checkDiamondKey($str)
{
	if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
	$c = $this->db->where('diamond_type_key',$str)->get($this->cTable)->num_rows();
	if($c > 0)
	{
		$this->form_validation->set_message('checkDiamondKey', 'This '.$str.' already exist in database, please try different.');
		return false;
	}
	else
		return true;
...
*/
	function diamondTypeForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('diamond_type_name','Diamond type name','trim|required|callback_checkDiamondType');
		if(!$this->cPrimaryId)
		{
			$this->form_validation->set_rules('diamond_type_key','Diamond type key ','trim|required|callback_checkDiamondKey');
		}
		...
		else
		{
			...
			{
				$this->dt->cPrimaryId = $this->cPrimaryId; // setting variable to model
				$this->dt->saveData();
 ...
 */	
	function updateSortOrder()
	{
		...
		if($id!=''  && $sort_order!='')
		{
			$this->db->where($this->cAutoId,$id)->update($this->cTable,array('dimaond_type_sort_order'=>$sort_order));
