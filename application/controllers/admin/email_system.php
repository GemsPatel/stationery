...
class email_system extends CI_Controller {
	
	var $is_ajax = false;
	var $cAutoId = 'email_send_id';
	var $cPrimaryId = '';
	var $cTable = 'email_send_history';
	var $controller = 'email_system';
	var $per_add = 1;
	var $per_edit = 1;
	var $per_delete = 1;
	var $per_view = 1;
	...
	{
		parent::__construct();
		$this->load->model('admin/mdl_email_system','es');
		$this->es->cTableName = $this->cTable;
		$this->es->cAutoId = $this->cAutoId;
		$this->is_ajax = $this->input->is_ajax_request();
		
		if($this->input->get('item_id') != '' || $this->input->post('item_id') != '')
			$this->cPrimaryId  = $this->es->cPrimaryId = _de($this->security->xss_clean($_REQUEST['item_id']));
		$this->chk_permission();		
...
*/	
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Email System', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
...
*/
	function sendEmailForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('es_from_emails','From email','trim|required');
		...
		else
		{
			...
			{
				$this->es->cPrimaryId = $this->cPrimaryId; // setting variable to model
				$this->es->sendEmails();
				if($this->input->post('private_email') != '')
					redirect('admin/customer_private_message');
				else
