...
class metal_color extends CI_Controller {
	
	var $is_ajax = false;
	var $cAutoId = 'metal_color_id';
	var $cPrimaryId = '';
	var $cTable = 'metal_color';
	var $controller = 'metal_color';
	var $per_add = 1;
	var $per_edit = 1;
	var $per_delete = 1;
	var $per_view = 1;
	...
	function metal_color()
	{
		parent::__construct();
		$this->load->model('admin/mdl_metal_color','mc');
...
*/	
	...
	function metalColorName($str)
	{
		
		if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('metal_color_name',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('metalColorName', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
	}
	function metalColorKey($str)
	{
		
		if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('metal_color_key',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('metalColorKey', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
...
*/
	function metalColorForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('metal_color_name','Metal Color name','trim|required|callback_metalColorName');
		if(!$this->cPrimaryId)
		{
			$this->form_validation->set_rules('metal_color_key','Metal Color Key','trim|required|callback_metalColorKey');
		}
		$this->form_validation->set_rules('metal_color_icon','Metal Color Icon','trim|required');
