...
class payment_method extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('admin/mdl_payment_method','shipme');
...
+--------------------------------------------------+
*/	
	function paymentMethodName($str)
	{
		if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('payment_method_name',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('paymentMethodName', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
...
+--------------------------------------------------+
*/	
	function paymentMethodKey($str)
	{
		if($this->cPrimaryId)
		$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('payment_method_key',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('paymentMethodKey', 'This '.$str.' already exist in database, please try different.');
			return false;
		}
		else
		return true;
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Payment Method', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
...
*/
	function paymentMethodForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('payment_method_name','Payment Method Name','trim|required|callback_paymentMethodName');
		if(!$this->cPrimaryId)
		{
			$this->form_validation->set_rules('payment_method_key','Payment Method Key','trim|required|callback_paymentMethodKey');
		}
		$this->form_validation->set_rules('payment_method_description','Payment Method Description','trim|required');
		$this->form_validation->set_rules('payment_method_icon','Payment Method Icon','trim|required');
 ...
 */	
	function updateSortOrder()
	{
		...
		if($id!=''  && $sort_order!='')
		{
			$this->db->where($this->cAutoId,$id)->update($this->cTable,array('payment_method_sort_order'=>$sort_order));
