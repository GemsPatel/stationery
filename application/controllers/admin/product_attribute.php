...
class product_attribute extends CI_Controller {
	...
	var $is_ajax = false;
	var $cAutoId = 'product_attribute_id';
	var $cPrimaryId = '';
	var $cTable = 'product_attribute';
	var $controller = 'product_attribute';
	var $per_add = 1;
	var $per_edit = 1;
	var $per_delete = 1;
	var $per_view = 1;
...
+--------------------------------------------------+
*/	
	function checkUniqueAttribute($str)
	{
		$c = null;
		$al  = url_title($str);
		if($this->cPrimaryId)
		...
		{
			$c = $this->db->where('pa_value',$al)->get($this->cTable)->num_rows();
		}
		else
		{
			$this->db->where("manufacturer_id =",MANUFACTURER_ID);
			$c = $this->db->where('pa_value',$al)->get($this->cTable."_cctld")->num_rows();
		}
		
		if($c > 0)
		{
			$this->form_validation->set_message('checkUniqueAttribute','Similar Attribute value is already exist. Try another one or append whitespace.');
			return false;
		}
		else
			return true;
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Product Attributes', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
		...
		$data['ims_filter'] = $this->input->get('ims_filter'); // filter by category name
		$data['it_filter'] = $this->input->get('it_filter'); // filter by category name
...
*/
	function productAttributeForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('inventory_master_specifier_id','Inventory Attribute','trim|required');
		$this->form_validation->set_rules('pa_value','Attribute Value','required');//|callback_checkUniqueAttribute 
		$this->form_validation->set_rules('pa_real_value','Applicable Value','trim|numeric');
 ...
 */	
	function updateSortOrder()
	{
		...
		if($id!=''  && $sort_order!='')
		{
			$this->db->where($this->cAutoId,$id)->update($this->cTable,array('pa_sort_order'=>$sort_order));
