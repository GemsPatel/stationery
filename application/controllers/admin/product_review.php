...
class product_review extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('admin/mdl_product_review','prore');
		$this->prore->cTableName = $this->cTable;
		$this->prore->cAutoId = $this->cAutoId;
		$this->is_ajax = $this->input->is_ajax_request();
		
		if($this->input->get('item_id') != '' || $this->input->post('item_id') != '')
			$this->cPrimaryId  = $this->prore->cPrimaryId = _de($this->security->xss_clean($_REQUEST['item_id']));
		$this->chk_permission();		
	}
/**
+----------------------------------------------------+
...
*/	
	...
	function index($stprore = 0)
	{
		...
		$num = $this->prore->getData();
		$data = pagiationData('admin/'.$this->controller,$num->num_rows(),$stprore,3);
		//echo $this->db->last_query();
		
		$data['stprore'] = $stprore; //stproreing position of records
		...
		$data['text_product'] = $this->input->get('text_product');// product search
...
*/
	function productReviewForm()
	{
		...
		$this->form_validation->set_rules('product_review_rating',' Product  Rating ','trim|required');
		$this->form_validation->set_rules('product_review_description','Product Review Description','trim|required');
		...
		else
		{
			...
			{
				$this->prore->cPrimaryId = $this->cPrimaryId; // setting variable to model
...
/*
+-----------------------------------------+
	Delete proreegory, single proreegory and multiple
	proreegory from single function call.
