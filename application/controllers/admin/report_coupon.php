...
class report_coupon extends CI_Controller {
	
	var $is_ajax = false;
	var $cAutoId = 'coupon_id';
	var $cTable = 'coupon';
	var $controller = 'report_coupon';
	var $per_view = 1;
	
	//parent constructor will load model inside it
	function report_coupon()
	{
		parent::__construct();
		$this->load->model('admin/report/mdl_report_coupon','rpco');
		$this->rpco->cTableName = $this->cTable;
		$this->rpco->cAutoId = $this->cAutoId;
...
*/	
	...
	function index($start = 0)
	{
		...
		$data['coupon_code_filter'] = $this->input->get('coupon_code_filter');
		$data['customer_name_filter'] = $this->input->get('customer_name_filter');
		$data['invoice_filter'] = $this->input->get('invoice_filter');
	...
	{
		$data['detail'] = $this->rpco->getCustomerDetails();
