...
class report_product_purchased extends CI_Controller {
	
	var $is_ajax = false;
	var $cAutoId = 'order_details_id';
	var $cTable = 'order_details';
	var $controller = 'report_product_purchased';
	var $per_view = 1;
	
	//parent constructor will load model inside it
	function report_product_purchased()
	{
		parent::__construct();
		$this->load->model('admin/report/mdl_report_product_purchased','rpp');
		$this->rpp->cTableName = $this->cTable;
		$this->rpp->cAutoId = $this->cAutoId;
...
*/	
	...
	function index($start = 0)
	{
		...
		$data['from_range_qty'] = $this->input->get('from_range_qty');
		$data['to_range_qty'] = $this->input->get('to_range_qty');
	...
	{
		$data['detail'] = $this->rpp->getproductDetails();
