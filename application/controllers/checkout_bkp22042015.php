...
*/	
	...
	{
		$d = $this->db->where('customer_emailid',$str)->get('customer')->row_array();
		if(count($d) > 0)
		{	
			$this->form_validation->set_message('checkMailDuplication','This email address already registered with us. <a class="cursor" onclick="$(\'#forgot_link\').click()">Forgot your password?</a>');
			return false;
		}		
		else
			return true;
