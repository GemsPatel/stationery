...
// 	    die;
	...
	function getHockertyData()
	{
		$hockertyArr = [ 'VEST'];
		
		foreach( $hockertyArr as $var )
		{
			$myfile = hefile_fileRead('hockerty/'.$var.'.txt');
			$readFileArr = explode( '<img onerror="" ' , $myfile);
			foreach ( $readFileArr as $k=>$ar )
			{
				$img = fetchSubStr( $ar , 'src="', '"');
				if( !empty( $img ) )
				{
					$content = file_get_contents( $img );
					file_put_contents( BASE_DIR . 'hockerty/'.$var.'/'.$k.'.png', $content);
				}
			}
		}
	}
	
	function importDistrictCode()
	{
	    $myfile = file_get_contents( BASE_DIR.$filename );
	    $readFileArr = explode( '<img onerror="" ' , $myfile);
	    foreach ( $readFileArr as $k=>$ar )
	    {
	        // 	        echo BASE_DIR . 'images/scrapper/'.$k.'.png';die;
	        $img = fetchSubStr( $ar , 'src="', '"');
	        if( !empty( $img ) )
	        {
	            $content = file_get_contents( $img );
	            file_put_contents( BASE_DIR . 'hockerty/CHESTPOCKET/'.$k.'.png', $content);
	        }
	    }
	}
}
