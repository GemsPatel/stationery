...
class mdl_country extends CI_Model
{
	...
	function getData()
	{
		if($this->cPrimaryId == "")
		{
			...
			if(isset($text_filter) && $text_filter != "")
				$this->db->where('country_name LIKE \''.$text_filter.'%\' ');
				
				
			if(isset($status_filter) && $status_filter != "")
				$this->db->where('country_status LIKE \''.$status_filter.'\' ');
	...
	function saveData()
	{
		...
		saveAdminLog($this->router->class, @$data['country_name'], $this->cTableName, $this->cAutoId, $last_id, $logType);
		setFlashMessage('success','Country has been '.(($this->cPrimaryId != '') ? 'updated': 'inserted').' successfully.');
...
*/	
	function deleteData($ids)
	{
		...
		if($ids)
		{
			foreach($ids as $id)
			{
				$tabNameArr = array('0'=>'customer_address','1'=>'state');
				$fieldNameArr = array('0'=>'country_id','1'=>'country_id');// this function call for un delete field
				...
				{
					$getName = getField('country_name', $this->cTableName, $this->cAutoId, $id);
