<?php
class mdl_customer_group extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function saveData()
	{
		...
		if($this->cPrimaryId != '')
		{
			$this->db->set('customer_group_modified_date', 'NOW()', FALSE);
		...
		{
			$data['customer_group_type'] = strtoupper($data['customer_group_type']);
			$this->db->insert($this->cTableName,$data);
			$last_id = $this->db->insert_id();
			$logType = 'A';
		}
		saveAdminLog($this->router->class, @$data['customer_group_name'], $this->cTableName, $this->cAutoId, $last_id, $logType); 
		setFlashMessage('success','Customer Group has been '.(($this->cPrimaryId != '') ? 'updated': 'inserted').' successfully.');
...
*/	
	function deleteData($ids)
	{
		...
		if($ids)
		{
			foreach($ids as $id)
			{
				$tabNameArr = array('0'=>'customer');
				$fieldNameArr = array('0'=>'customer_group_id');
				...
				{
					$getName = getField('customer_group_name', $this->cTableName, $this->cAutoId, $id);
...
*/	
	function updateStatus()
	{
		...
		$data['customer_group_status'] = $status;
