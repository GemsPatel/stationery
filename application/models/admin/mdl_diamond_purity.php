<?php
class mdl_diamond_purity extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function saveData()
	{
		...
		if($this->cPrimaryId != '')
		{
			unset($data['diamond_purity_key']);
			$this->db->set('diamond_purity_modified_date', 'NOW()', FALSE);
		...
		{
			$data['diamond_purity_key'] = strtoupper($data['diamond_purity_key']);
			$this->db->insert($this->cTableName,$data);
			$last_id = $this->db->insert_id();
			$logType = 'A';
		}
		saveAdminLog($this->router->class, @$data['diamond_purity_name'], $this->cTableName, $this->cAutoId, $last_id, $logType);
		setFlashMessage('success','Diamond purity has been '.(($this->cPrimaryId != '') ? 'updated': 'inserted').' successfully.');
	...
	function deleteData($ids)
	{
		...
		if($ids)
		{
			foreach($ids as $id)
			{
					...
					$getName = getField('diamond_purity_name', $this->cTableName, $this->cAutoId, $id);
...
*/	
	function updateStatus()
	{
		...
		$data['diamond_purity_status'] = $status;
