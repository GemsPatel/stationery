<?php
class mdl_languages extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function saveData()
	{
		...
		{
			$data['l_key'] = strtoupper($data['l_key']);
			$this->db->insert($this->cTableName,$data);
			$last_id = $this->db->insert_id();
			$logType = 'A';
		}
		saveAdminLog($this->router->class, @$data['l_name'], $this->cTableName, $this->cAutoId, $last_id, $logType); 
		setFlashMessage('success','languages has been '.(($this->cPrimaryId != '') ? 'updated': 'inserted').' successfully.');
...
*/	
	function deleteData($ids)
	{
		...
		if($ids)
		{
			...
			{
				/**
				 * CHANGE ON 17-Apr-15 Not Delete Language Error 'ch_abstract_auto_responder' not in field list
				 */
// 				$tabNameArr = array('0'=>'ch_abstract_auto_responder');
// 				$fieldNameArr = array('0'=>'abstract_auto_responder_id');
...
// 				else
// 				{
					$getName = getField('l_name', $this->cTableName, $this->cAutoId, $id);
