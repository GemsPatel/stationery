<?php
class mdl_metal_purity extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function saveData()
	{
		...
		if($this->cPrimaryId != '')
		{
			$this->db->set('metal_purity_modified_date', 'NOW()', FALSE);
		...
		{
			$data['metal_purity_key'] = strtoupper($data['metal_purity_key']);
			$this->db->insert($this->cTableName,$data);
			$last_id = $this->db->insert_id();
			$logType = 'A';
		}
		saveAdminLog($this->router->class, @$data['metal_purity_name'], $this->cTableName, $this->cAutoId, $last_id, $logType); 
		setFlashMessage('success','Metal Purity has been '.(($this->cPrimaryId != '') ? 'updated': 'inserted').' successfully.');
	...
	function deleteData($ids)
	{
		...
		if($ids)
		{
			foreach($ids as $id)
			{
				...
				$fieldNameArr = array('0'=>'metal_purity_id');
				...
				{
						$getName = getField('metal_purity_name', $this->cTableName, $this->cAutoId, $id);
...
*/	
	function updateStatus()
	{
		...
		$data['metal_purity_status'] = $status;
