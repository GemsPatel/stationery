<?php
class mdl_metal_type extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function saveData()
	{
		...
		if($this->cPrimaryId != '')
		{
			$this->db->set('mt_modified_date', 'NOW()', FALSE);
		...
		{
			$data['metal_type_key'] = strtoupper($data['metal_type_key']);
			$this->db->insert($this->cTableName,$data);
			$last_id = $this->db->insert_id();
			$logType = 'A';
		}
		saveAdminLog($this->router->class, @$data['metal_type_name'], $this->cTableName, $this->cAutoId, $last_id, $logType); 
		setFlashMessage('success','Metal Type has been '.(($this->cPrimaryId != '') ? 'updated': 'inserted').' successfully.');
	...
	function deleteData($ids)
	{
		...
		if($ids)
		{
			foreach($ids as $id)
			{
				...
				$fieldNameArr = array('0'=>'metal_type_id');
				...
				{
					$getName = getField('metal_type_name', $this->cTableName, $this->cAutoId, $id);
