<?php
class mdl_order_status extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function getData()
	{
		if($this->cPrimaryId == "")
		{
			...
			$staus_name = $this->input->get('staus_name');
			$staus_key = $this->input->get('staus_key');
			
			if(isset($staus_name) && $staus_name != "")
				$this->db->where('order_status_name LIKE \''.$staus_name.'%\' ');
				
			if(isset($staus_key) && $staus_key != "")
				$this->db->where('order_status_key LIKE \'%'.$staus_key.'%\' ');
				
			if(isset($status_filter) && $status_filter != "")
				$this->db->where('order_status_status LIKE \''.$status_filter.'\' ');
	...
	function saveData()
	{
		...
		$data['order_status_key'] = strtoupper($data['order_status_key']);
		$getImg = getField('order_status_icon', $this->cTableName, $this->cAutoId, $this->cPrimaryId);
		
		if($this->input->post('order_status_icon') && $_FILES['order_status_icon']['name'])
		{
			$data['order_status_icon'] = $this->resizeUploadImage(); //upload and resize image		
			if($getImg != '')
				@unlink($getImg);
		}
		
		if($this->input->post('order_status_icon') && $_FILES['order_status_icon']['name'] == '')
			$data['order_status_icon'] = $this->input->post('order_status_icon');
		...
		if($this->cPrimaryId != '')
		{
			unset($data['order_status_key']);
			$this->db->set('order_status_modified_date', 'NOW()', FALSE);
		...
		saveAdminLog($this->router->class, @$data['order_status_name'], $this->cTableName, $this->cAutoId, $last_id, $logType);
...
*/	
	function deleteData($ids)
	{
		...
		if($ids)
		{
			...
			foreach($ids as $id)
			{
				$tabNameArr = array('0'=>'order_tracking','1'=>'email_send_history','2'=>'order_return');
				$fieldNameArr = array('0'=>'order_status_id','1'=>'es_status','2'=>'order_status_id');
				...
				else
				{
					...
					@unlink($getImg);
					$getName = getField('order_status_name', $this->cTableName, $this->cAutoId, $id);
...
*/	
	function updateStatus()
	{
		...
		$data['order_status_status'] = $status;
