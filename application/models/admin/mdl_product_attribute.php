...
class mdl_product_attribute extends CI_Model
{
	...
	function saveData()
	{
		...
		$last_id = $this->saveItem( $data, $logType );
	...
	function saveItem( $data, &$logType)
	{
		// post data for insert and edit
		...
		{
			$data["pa_value"] = validateInput( $data["pa_value"] );
		}
	
		//
		if( isset($data["pa_extra_value1"]) )
		{
			$data["pa_extra_value1"] = validateInput( $data["pa_extra_value1"] );
		}
		
		if( isset($data["pa_extra_value2"]) )
		{
			$data["pa_extra_value2"] = validateInput( nl2emty_custom( $data["pa_extra_value2"] ) );
		}
		
		if( isset($data["pa_extra_value3"]) )
		{
			$data["pa_extra_value3"] = validateInput( $data["pa_extra_value3"] );
		}
		
		if( isset( $data["pa_real_value"] ) && !empty( $data["pa_real_value"] ) )
		{
			$data["pa_real_value"] = validateInput( $data["pa_real_value"] );
		}
		else 
		{
			unset( $data["pa_real_value"] );
		}
	
		if( empty( $data["pa_sort_order"] ) )
		{
			$data["pa_sort_order"] = 0;
		}
		
		$getImg="";
		if( $this->cPrimaryId != '' )
		{
			$getImg = exeQuery( " SELECT pa_icon FROM ".$this->cTableName." WHERE ".$this->cAutoId."=".$this->cPrimaryId." ", true, 'pa_icon' );
		}
		if($this->input->post('pa_icon') && $_FILES['pa_icon']['name'])
		{
			$data['pa_icon'] = $this->resizeUploadImage(); //upload and resize image
			if($getImg != ''){
				//@unlink($getImg);
			}
		}
		...
		if($this->input->post('pa_icon') && $_FILES['pa_icon']['name'] == '')
			$data['pa_icon'] = $this->input->post('pa_icon');
		...
		//allowed ordering for all on 03-03-2016
		if(
		TRUE
		||
		$data["inventory_master_specifier_id"] == inventroyAttributeIdByKey("DENOMATION_SEO")
		)
		{
			// 			SET @x = 0;
			...
			$this->db->query('UPDATE product_attribute SET `pa_sort_order` = (@x:=@x+5) WHERE `inventory_master_specifier_id`='.(int)$data["inventory_master_specifier_id"].' ORDER BY `pa_value`;');
