<?php
class mdl_shipping_method extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function getData()
	{
		if($this->cPrimaryId == "")
		{
			...
			$text_free = $this->input->get('text_free');
			$text_charge = $this->input->get('text_charge');
		
			if(isset($text_name) && $text_name != "") // text box value search
				$this->db->where('shipping_method_name LIKE \''.$text_name.'%\' ');
			if(isset($text_key) && $text_key != "") // text box value search
				$this->db->where('shipping_method_key LIKE \''.$text_key.'%\' ');	
			
			if(isset($text_free) && $text_free != "") // text box value search
				$this->db->where('shipping_method_free_shipping LIKE \''.$text_free.'%\' ');
				if(isset($text_charge) && $text_charge != "") // text box value search
				$this->db->where('shipping_method_handling_charges LIKE \''.$text_charge.'%\' ');
						
			if(isset($status_filter) && $status_filter != "") // status wise fiter 
				$this->db->where('shipping_method_status LIKE \''.$status_filter.'\' ');
	...
	function saveData()
	{
		...
		$getImg = getField('shipping_method_icon', $this->cTableName, $this->cAutoId, $this->cPrimaryId);
		
		if($this->input->post('shipping_method_icon') && $_FILES['shipping_method_icon']['name'])
		{
			$data['shipping_method_icon'] = $this->resizeUploadImage(); //upload and resize image		
			if($getImg != '')
				@unlink($getImg);
		}
		
		if($this->input->post('shipping_method_icon') && $_FILES['shipping_method_icon']['name'] == '')
			$data['shipping_method_icon'] = $this->input->post('shipping_method_icon');
		...
		if($this->cPrimaryId != '')
		{
			$this->db->set('shipping_method_modified_date', 'NOW()', FALSE);
		...
		{
			$data['shipping_method_key'] = strtoupper($data['shipping_method_key']);
			$this->db->insert($this->cTableName,$data);
			$last_id = $this->db->insert_id();
			$logType = 'A';
		}
		saveAdminLog($this->router->class, @$data['shipping_method_name'], $this->cTableName, $this->cAutoId, $last_id, $logType);
	...
	function deleteData($ids)
	{
		...
		{		
			...
			foreach($ids as $id)
			{
				...
				$fieldNameArr = array('0'=>'shipping_method_id');
				...
				{	
					   	 $getImg = getField('shipping_method_icon', $this->cTableName, $this->cAutoId, $id);
						 @unlink($getImg);
						 $getName = getField('shipping_method_name', $this->cTableName, $this->cAutoId, $id);
...
*/	
	function updateStatus()
	{
		...
		$data['shipping_method_status'] = $status;
...
*/	
	function resizeUploadImage()
	{
		...
		{
			$image = uploadFile('shipping_method_icon','image','shipping_method'); //input file, type, folder
			if(@$image['error'])
			{
