<?php
class mdl_stock_status extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cPrimaryId = '';
	...
	function saveData()
	{
		...
		saveAdminLog($this->router->class, @$data['status_name'], $this->cTableName, $this->cAutoId, $last_id, $logType);
		setFlashMessage('success','Stock Status has been '.(($this->cPrimaryId != '') ? 'updated': 'inserted').' successfully.');
...
*/	
	function deleteData($ids)
	{
		...
		if($ids)
		{
			foreach($ids as $id)
			{   
				$tabNameArr = array('0'=>'product_value');
				$fieldNameArr = array('0'=>'stock_status_id');
				...
				{
					$getName = getField('status_name', $this->cTableName, $this->cAutoId, $id);
