<?php
class mdl_report_customer_cart extends CI_Model
{
	var $cTableName = '';
	var $cAutoId = '';
	var $cCategory = '';
	...
	function getData()
	{
		if($this->cPrimaryId == "")
		{
			...
			if(isset($product_code) && $product_code != "")
				$this->db->where('product_price.product_generated_code LIKE \''.$product_code.'%\'' );	
		...
		$this->db->where('customer_cartwish_type','C');
