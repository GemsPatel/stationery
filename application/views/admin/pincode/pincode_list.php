      ...
      <div class="buttons">
       <form method="post" id="import_form" enctype="multipart/form-data" action="<?php echo site_url('admin/'.$this->controller.'/importData') ?>">
          <input type="file" name="import_csv" />
           <a class="button" title="format:: pincode,areaname,cityname,state_id" href="javascript:void(0)" onclick="$('#import_form').submit();">Import</a>
       <?php if($this->per_add == 0):?>
      		<a class="button" href="<?php echo site_url('admin/'.$this->controller.'/pincodeForm')?>">Insert</a>
        <?php endif;?>
