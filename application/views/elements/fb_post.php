
<div id="fb-root"></div>
<script type="text/javascript">
// for local 161130247380489 //for live 184206418396629

var isLoaded = false;
  window.fbAsyncInit = function() {
    
	FB.init({
      appId      : '<?php echo FB_APP_ID ?>', // App ID
      status     : true, // check login status
      cookie     : true, // enable cookies to allow the server to access the session
      xfbml      : true  // parse XFBML
    });
    isLoaded = true;

    // Additional initialization code here
  };
  // Load the SDK Asynchronously
  (function(d){
     var js, id = 'facebook-jssdk', ref = d.getElementsByTagName('script')[0];
     if (d.getElementById(id)) {return;}
     js = d.createElement('script'); js.id = id; js.async = true;
     js.src = "//connect.facebook.net/en_US/all.js";
     ref.parentNode.insertBefore(js, ref);
   }(document));

function facebook_post() 
{
	if(!isLoaded) 
	{
		alert("JS-SDK is not yet loaded. Try again after few seconds.");
		return false;
	}
	FB.login(function(response) 
	{
		if (response.authResponse) 
		{
			var fb_access_token = response.authResponse.accessToken;
			
			FB.api('/me', function(response) 
			{
				document.getElementById('fb_user_fname').value = response.first_name;
				document.getElementById('fb_user_lname').value = response.last_name;
				document.getElementById('fb_user_email').value = response.email;
				document.getElementById('fb_facebook_id').value = response.id;
				document.getElementById('fb_facebook_offline_token').value = fb_access_token;
				document.facebook_fbpost_form.submit();
			});
		}
	},{scope: 'email,publish_stream,user_birthday,status_update,offline_access'});
	return false;
}

</script>
<form method="post" name="facebook_fbpost_form" action="<?php echo site_url('home/shareWallPost');?>">
<input type="hidden" name="fb_user_fname" id="fb_user_fname" value="" />
<input type="hidden" name="fb_user_lname" id="fb_user_lname" value="" />
<input type="hidden" name="fb_user_email" id="fb_user_email" value="" />
<input type="hidden" name="fb_facebook_id" id="fb_facebook_id" value="" />
<input type="hidden" name="fb_facebook_offline_token" id="fb_facebook_offline_token" value="" />
<input type="hidden" name="url" id="url" value="<?php echo 'http://www.perrian.com/jewellery/diamonds/earrings/the-pernel-drop-earrings/170366';//$url ?>" />
<input type="hidden" name="title" id="title" value="<?php echo 'The Pernel Drop Earrings';//$title ?>" />
<input type="hidden" name="imgUrl" id="imgUrl" value="<?php echo 'http://www.perrian.com/assets/product/PER-225/PEARL/CHI_WAT_PEARL/SIDESTONE1/DIAMOND/DIAMOND-A/YELLOW/PER-225-Y1.jpg';//$imgUrl ?>" />
<input type="hidden" name="description" id="description" value="<?php echo @$description .' '.@$url ?>" />
</form>